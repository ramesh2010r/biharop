              ...
              <div className="relative z-10 p-6 md:p-10">
                ...
                {loading && selectedDistrict && (
                  <div className="mt-6 text-center py-8">
                    ...
                    <p className="mt-4 text-white hindi-text">परिणाम लोड हो रहे हैं...</p>
                  </div>
                )}

                {/* Error Message inside card */}
                {error && selectedConstituency && (
                  <div className="mt-6 bg-red-500/20 backdrop-blur-sm border-2 border-red-300/50 p-4 rounded-xl">
                    <p className="text-white hindi-text font-medium text-center">{error}</p>
                  </div>
                )}

                {/* Results Display inside the same card */}
                {!loading && results.length > 0 && selectedConstituency && (
                  <div className="mt-6">
                    ...
                    {/* Results Header */}
                    <h3 className="text-xl md:text-2xl font-bold text-white hindi-text mb-6 text-center">
                      मतदान परिणाम
                    </h3>
                    
                    {/* Results List with White Cards */}
                    <div className="space-y-3 md:space-y-4">
                      {results.map((result, index) => (
                        <div key={index} className="bg-white/95 backdrop-blur-sm rounded-xl p-4 md:p-5 shadow-lg hover:shadow-xl transition-all duration-300">
                          <div className="flex justify-between items-center mb-3">
                            <div className="flex-1 pr-4">
                              <h4 className="font-bold text-gray-900 hindi-text text-base md:text-lg">
                          ...
                          <div className="w-full bg-gray-200 rounded-full h-3 md:h-4 overflow-hidden">
                            <div
                              className="bg-gradient-to-r from-blue-500 to-blue-600 h-full rounded-full transition-all duration-700 shadow-inner"
                    ...
                    <div className="mt-6 pt-4 border-t border-white/20">
                      <p className="text-sm md:text-base text-white/90 hindi-text text-center font-medium">
                ...
                {/* Share Section - Only show when results exist */}
                {!loading && results.length > 0 && selectedConstituency && (
                  <div className="mt-6">
                    {/* Divider */}
                    <div className="h-px bg-white/30 mb-6"></div>
                    ...
                    <div className="py-4">
                      <h3 className="text-lg md:text-xl font-semibold text-white hindi-text mb-3 text-center">
                        परिणाम साझा करें
                      </h3>
                      <p className="text-sm text-white/80 hindi-text text-center mb-4">
                        इन परिणामों को अपने मित्रों के साथ साझा करें
                      </p>

                      {/* Social Media Share - Compact Grid */}
                      <div className="pt-2">
                        <p className="text-xs md:text-sm text-white/90 mb-3 hindi-text text-center">सोशल मीडिया पर शेयर करें:</p>
                        ...
                        {!isIOS && results.length > 0 && (
                          <div className="mt-4 flex justify-center">
                            <button
                              onClick={handleDownloadImage}
                              className="px-4 py-2 rounded-lg bg-white/20 hover:bg-white/30 text-white font-bold hindi-text transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5 flex items-center gap-2 text-sm backdrop-blur-sm border border-white/30"
